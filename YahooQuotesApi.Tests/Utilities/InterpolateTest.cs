...
namespace YahooQuotesApi.Tests
{
    public class InterpolateTest
    {
        ...
        public void TestNotEnoughData()
        {
            ...
            Assert.Throws<ArgumentException>(() => list.Interpolate(new ZonedDateTime().ToInstant()));
            list.Add(new PriceTick(new ZonedDateTime(), 0));
            Assert.Throws<ArgumentException>(() => list.Interpolate(new ZonedDateTime().ToInstant()));
            list.Add(new PriceTick(new ZonedDateTime(), 0));
            Assert.Equal(0, list.Interpolate(new ZonedDateTime().ToInstant()));
            list.Add(new PriceTick(new ZonedDateTime(), 0));
            Assert.Equal(0, list.Interpolate(new ZonedDateTime().ToInstant()));
        }
        ...
        public void BoundaryTest()
        {
            ...
            var result = list.Interpolate(zdt1.ToInstant());
            Assert.False(double.IsNaN(result)); // enough data

            result = list.Interpolate(zdt1.PlusHours(-7 * 24).ToInstant());
            Assert.True(double.IsNaN(result)); // not enough data

            result = list.Interpolate(zdt2.ToInstant());
            Assert.False(double.IsNaN(result)); // enough data

            result = list.Interpolate(zdt2.PlusHours(7 * 24).ToInstant());
            Assert.True(double.IsNaN(result)); // not enough data
        }
        ...
        public void BoundaryLimitTest()
        {
            ...
            var result = list.Interpolate(zdt2.PlusTicks(1).ToInstant());
        ...
        public void InterpolateTest1()
        {
            ...
            var result = list.Interpolate(zdt1.Plus(Duration.FromDays(1)).ToInstant());
